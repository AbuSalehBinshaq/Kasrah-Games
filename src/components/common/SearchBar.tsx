...
export default function SearchBar({ initialValue = '', value, onChange, placeholder = 'Search for games...' }: SearchBarProps) {
  const [search, setSearch] = useState(initialValue);
  const router = useRouter();
  const isControlled = value !== undefined && onChange !== undefined;
  const currentValue = isControlled ? value : search;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (currentValue.trim()) {
      router.push(`/games?search=${encodeURIComponent(currentValue.trim())}`);
    }
  };
